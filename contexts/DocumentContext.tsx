...
export function DocumentProvider({ children }: DocumentProviderProps) {
    ...
    try {
      ...
      if (storedDocument) {
        ...
        if (parsed.success) {
          ...
          messenger.message(
            `Stored document failed validation: ${parsed.error}\nUsing default document.`,
            MessageType.Error
          );
      ...
      messenger.message(
        `Error loading document from storage: ${error}`,
        MessageType.Error
      );
    } finally {
      setIsLoading(false);
  ...
  const saveDocument = useCallback(
    async (documentToSave: CvDocument) => {
      try {
        ...
        messenger.message(
          `Error saving document to storage: ${error}`,
          MessageType.Error
        );
      }
    },
  ...
  const exportDocument = useCallback(
    async (fileName: string = DEFAULT_EXPORT_FILE_NAME) => {
      try {
        ...
        messenger.message(
          'Export is only implemented for web at the moment.',
          MessageType.Warn
        );
      } catch (error) {
        messenger.message(
  ...
  const importDocument = useCallback(async () => {
    ...
    if (!first) {
      messenger.message('No file selected for import', MessageType.Error);
      return;
    }
    ...
    if (first.file && 'text' in first.file) {
      ...
      messenger.message(
        'Import is only implemented for web at the moment.',
        MessageType.Warn
      );
    }
    ...
    if (!text) {
      messenger.message(
        'No text content found in the selected file',
        MessageType.Error
      );
      return;
    }
   ...
   */
  const updateDocument = useCallback((updates: Partial<CvDocument>) => {
  ...
  }, [documentData, isLoading, saveDocument]);
